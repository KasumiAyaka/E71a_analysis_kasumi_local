...
int main(int argc, char** argv) {
	...
	try {
		...
		std::cerr << "filesystem::copy: " << ec.what() << std::endl;
		std::exit(1);
	}
...
#pragma omp critical
	...
	auto end = std::chrono::system_clock::now(); // ?v???J?n????
	double elapsed = std::chrono::duration_cast<std::chrono::seconds>(end - start).count(); //??????v??????????~???b????
...
int do_hts_ali_one(std::string filepath) {
	...
	command[0] << "Z:\\usr\\Project_v150\\htstools_v142_hts2\\hts_beta_ali_v2.exe " << filepath << "\\beta_thick_0.dat ali.json --stage_type 2 --view_step_x 9 --view_step_y 5 --th_pos2 0.0025";//2023/03/14
...
int do_hts2beta_one(std::string file_in_data, std::string file_out_data, int arg, int flg) {
	...
	HANDLE h_out;
	//hts2beta start
	auto hts2beta_start = std::chrono::system_clock::now(); // ?v???J?n????
	...
	auto hts2beta_end = std::chrono::system_clock::now(); // ?v???J?n????
	double elapsed = std::chrono::duration_cast<std::chrono::seconds>(hts2beta_end - hts2beta_start).count(); //??????v??????????~???b????
...
PROCESS_INFORMATION do_prg(std::string path, std::string command, std::string output_log, HANDLE& h_out) {
	...
	int ret = CreateProcess(
		NULL, // ???s??\???W???[?????
		str, // ?R?}???h???C?????????
		&sec_attr, // ?Z?L?????e?B?L?q?q
		&sec_attr,// ?Z?L?????e?B?L?q?q
		TRUE, // ?n???h????p???I?v?V????
		NORMAL_PRIORITY_CLASS, // ????t???O
		NULL,// ?V????????u???b?N
		path.c_str(), // ?J?????g?f?B???N?g??????O
		&s, // ?X?^?[?g?A?b?v???
		&p // ?v???Z?X???
	);
	if (!ret)
