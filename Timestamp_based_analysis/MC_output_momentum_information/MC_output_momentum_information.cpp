...
public:
	...
	int charge;
	int BMmomflg;// 
	double BMmom,BMerrp,BMerrm;
	double MCS, MCSerrp, MCSerrm;
};
class track_pair {
public:
	int eventid;
...
int main(int argc, char** argv) {
	if (argc < 4) {
		fprintf(stderr, "usage:prg MC.txt out-vtx.txt #ECC\n");
		fprintf(stderr, "usage:prg MC.txt out-vtx.txt #ECC 1\n-->does not display logs...\n");
		exit(1);
	}
	...
	while (std::getline(ifs, str)) {
		...
		if (str_v.size() == 27) {
			...
			if (int(std::stoi(str_v[2]) / 10000) > 0) {// recon
				...
				dflg = std::stoi(str_v[3]);//direction 1:fwd
				stop_tmp.charge = std::stoi(str_v[4]);
				//MCS
				stop_tmp.MCS= std::stod(str_v[7]);
				stop_tmp.MCSerrm = std::stod(str_v[15]);
				stop_tmp.MCSerrp = std::stod(str_v[16]);
				//BM mom
				stop_tmp.BMmom = std::stod(str_v[9]);
				stop_tmp.BMerrm = std::stod(str_v[19]);
				stop_tmp.BMerrp = std::stod(str_v[20]);
				// MCS liklihood
			...
			/*			
			if (int(std::stoi(str_v[2]) / 10000) <= 0) {// true
				flg++;
...
void clustering_2trk_vtx2_ver2(std::multimap<int, stop_track>& tracks, int pl, std::ofstream& ofs, int ecc, int mode) {
	...
	for (auto itr0 = ret.begin(); itr0 != ret.end(); itr0++) {
		for (auto itr1 = itr0->trk.begin(); itr1 != itr0->trk.end(); itr1++) {
			...
			if (itr1->second.x < 40000 || itr1->second.x>210000 || itr1->second.y < 40000 || itr1->second.y>210000)continue;
			if (fabs(itr1->second.ax )> 4.0000 ||  fabs(itr1->second.ay) > 4.0000 )continue;
			...
			ofs << std::right << std::fixed
				...
				<< std::setw(10) << std::setprecision(1) << itr1->second.x << " "
				<< std::setw(10) << std::setprecision(1) << itr1->second.y << " "
				<< std::setw(2) << std::setprecision(0) << itr1->second.charge << " "
				<< std::setw(8) << std::setprecision(1) << itr1->second.MCS << " "
				<< std::setw(8) << std::setprecision(1) << itr1->second.MCSerrm << " "
				<< std::setw(8) << std::setprecision(1) << itr1->second.MCSerrp << " "
				<< std::setw(8) << std::setprecision(1) << itr1->second.BMmom << " "
				<< std::setw(8) << std::setprecision(1) << itr1->second.BMerrm << " "
				<< std::setw(8) << std::setprecision(1) << itr1->second.BMerrp << " "
				<< std::setw(9) << std::setprecision(4) << itr1->second.weight
				<< std::endl;
		}
	}
