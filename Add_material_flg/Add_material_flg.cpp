...
void add_vertex_material_flg(std::vector<Momentum_recon::Event_information>& momch, std::map<int, int>& list);
...
int main(int argc, char** argv) {
	...
	std::string in_list = argv[3];// material
	...
	std::cout << " * set material flg " << std::endl;
	// read flg
	std::map<int, int>ml;
	Set_manual_check_result(in_list, ml);

	// add vertex material
	std::cout << " * add material flg " << std::endl;
	add_vertex_material_flg(momch, ml);
...
void Set_manual_check_result(std::string input, std::map<int, int>& list) {
	...
	if (!ifs) {
		std::cerr << "\nFailed to set manual check results." << std::endl;
		exit(0);
	}
	...
	std::cout << "    Overwrite event list" << std::endl;
	while (std::getline(ifs, str)) {

		str_v = StringSplit_with_tab(str);
		if (list.find(std::stoi(str_v[0])) == list.end()) {
			list.insert(std::make_pair(std::stoi(str_v[0]), std::stoi(str_v[1])));// group id
		}
		else {
			if (std::stoi(str_v[1]) < 0) {
				// overwrite
				std::cout << "\t" << std::stoi(str_v[0]) << ": " << list.find(std::stoi(str_v[0]))->second << " --> " << std::stoi(str_v[1]) << std::endl;
				list.find(std::stoi(str_v[0]))->second = std::stoi(str_v[1]);
			}
		}
...
void add_vertex_material_flg(std::vector<Momentum_recon::Event_information>& momch, std::map<int,int>& list) {
	...
	for (auto& ev : momch) {
		...
		if (list.count(ev.groupid) > 0) {
			ev.vertex_material = list.find(ev.groupid)->second;
			cnt++;
		}
		std::cout << std::right << std::fixed
			<< std::setw(10) << std::setprecision(0) << ev.groupid << " "
			<< std::setw(10) << std::setprecision(0) << ev.unix_time << " "
			<< std::setw(3) << std::setprecision(0) << ev.vertex_material
			<< std::endl;
