...
void ADD_weight(std::vector<Momentum_recon::Event_information>& momch, std::string in);
...
void ADD_weight(std::vector<Momentum_recon::Event_information>& momch, std::string in) {
	...
	std::ifstream ifs(in);
	while (ifs >> k >> w) {
		map.insert(std::make_pair(k, w));
		std::cout << k << " " << w << std::endl;
	}

	for (auto& ev : momch) {
		auto a = map.find(ev.groupid);
		std::cout << ev.groupid << " " << ev.weight;
