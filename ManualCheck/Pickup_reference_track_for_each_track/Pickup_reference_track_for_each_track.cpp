// 2026/08/25
// マニュアルチェックする飛跡ごとにリファレンストラックを一本選んでくる
// kasumi
...
void Search_ref_btrk(std::string filename, int pl, std::ofstream& ofs, int ecc, int zone) {
	...
	do {
		...
		area.push_back(vxx::CutArea(pos[0] - d, pos[0] + d, pos[1] - d, pos[1] + d, -1.0, 1.0, -1.0, 1.0));//xmin, xmax, ymin, ymax, axmin, axmax, aymin, aymax
		std::vector<vxx::base_track_t> base = br.ReadAll(filename, pl, 0, vxx::opt::ph = 32, vxx::opt::a = area);
		std::vector<vxx::base_track_t> base_selected;
		const int NumberOfImager = 72;
		uint32_t ShotID;
		...
		// 要素が0 --> 探索領域を広げて再探索
		if (base_selected.size() < 1) {
			std::cout << "条件を満たすtrackが存在しません。" << std::endl;
			d = d + 2000;
