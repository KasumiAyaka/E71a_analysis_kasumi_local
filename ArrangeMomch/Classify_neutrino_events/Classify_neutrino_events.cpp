...
void clustering_2trk_vtx(std::multimap<int, stop_track>& tracks, int pl, std::map<int, vtx_point>& vtx, std::map<int, DivideParam>& flg, double thr_md);
void DivideMomch(std::vector<Momentum_recon::Event_information>& momch, std::map<int, DivideParam>& flg, std::vector<Momentum_recon::Event_information>& momch_single, std::vector<Momentum_recon::Event_information>& momch_neuint, std::vector<Momentum_recon::Event_information>& momch_others, int trknum, std::string log);

int main(int argc, char** argv) {
	if (argc != 5) {
		fprintf(stderr, "usage:prg in.momch output_path thr_md[um] thr_trknum(without mu+p)\n");
		exit(1);
	}
	std::string in_momch = argv[1];
	std::string out_momch = argv[2];// dir path
	double thr_md = std::stod(argv[3]);
	int thr_trk = std::stoi(argv[4]);
	...
	std::stringstream name1, name2, name3, log;

	std::vector<Momentum_recon::Event_information> momch_single;
	std::vector<Momentum_recon::Event_information> momch_neuint;
	std::vector<Momentum_recon::Event_information> momch_others;
	log << out_momch << "\\ClassificationResults.txt";
	std::string log_txt = log.str();
	DivideMomch(momch, flg, momch_single, momch_neuint, momch_others, thr_trk, log_txt);
	// set folder name
	name1 << out_momch << "\\Single_Muon.momch";
	name2 << out_momch << "\\Neuint_Like.momch";
	name3 << out_momch << "\\OtherEvents.momch";
	//"\\pl" << std::setw(3) << std::setfill('0') << itr->pl << "e" << std::setw(8) << std::setfill('0') << itr->eid << ".all";
	std::string output1 = name1.str();
	std::string output2 = name2.str();
	std::string output3 = name3.str();

	Momentum_recon::Write_Event_information_extension(output1, momch_single);
	Momentum_recon::Write_Event_information_extension(output2, momch_neuint);
	Momentum_recon::Write_Event_information_extension(output3, momch_others);
...
void clustering_2trk_vtx(std::multimap<int, stop_track>& tracks, int pl, std::map<int, vtx_point>& vtx, std::map<int, DivideParam>& flg, double thr_md) {
	...
	std::cout << tracks.size() << std::endl;
				fg.pnum = 0; fg.tnum = tracks.size(); fg.md = 0;
if (tracks.size() != 1) {
		for (auto itr1 = tracks.begin(); itr1 != tracks.end(); itr1++) {
			...
			for (auto itr0 = tracks.begin(); itr0 != tracks.end(); itr0++) {
				...
				else if (itr0->second.pid == 2212) {
					// proton
					fg.pnum++;
				}
				else if (itr0->second.pid == 211) {
					// pion
				}
				else {
	...
	flg.insert(std::make_pair(multi.eventid, fg));
...
void DivideMomch(std::vector<Momentum_recon::Event_information>& momch, std::map<int, DivideParam>& flg, std::vector<Momentum_recon::Event_information>& momch_single,
	std::vector<Momentum_recon::Event_information>& momch_neuint, std::vector<Momentum_recon::Event_information>& momch_others, int trknum, std::string log) {

	std::ofstream ofs(log);
	ofs << "1:single-muon, 2:neutrino-like, 3:others" << std::endl;
	for (auto& ev : momch) {
		auto itr = flg.find(ev.groupid);
		//std::cout << itr->second.tnum << " " << itr->second.pnum << " " << itr->second.md << std::endl;
		if (itr->second.tnum == 1) {
			// single muon event
			momch_single.push_back(ev);
			ofs << ev.groupid << " " << ev.vertex_pl<<" " << 1 << std::endl;
		}
		else {
			// multi track vertex
			if (itr->second.pnum > 0) {
				// at least 1 proton
				if (itr->second.tnum >= 2 + trknum && itr->second.md > 1) {
					// mu+p+other-trk and mu+p,mu+othre-trk < thr_md
					momch_neuint.push_back(ev);
					ofs << ev.groupid << " " << ev.vertex_pl << " " << 2 << std::endl;
				}
				else {
					// others
					momch_others.push_back(ev);
					ofs << ev.groupid << " " << ev.vertex_pl << " " << 3 << std::endl;
				}
			}
			else {
				// events without proton := others
				momch_others.push_back(ev);
				ofs << ev.groupid << " " << ev.vertex_pl << " " << 3 << std::endl;
			}
		}
	}
	std::cout << " * Single muon event : " << momch_single.size() << std::endl;
	std::cout << " * Neuint like event : " << momch_neuint.size() << std::endl;
	std::cout << " * Other events      : " << momch_others.size() << std::endl;
	ofs << " * Single muon event : " << momch_single.size() << std::endl;
	ofs << " * Neuint like event : " << momch_neuint.size() << std::endl;
	ofs << " * Other events      : " << momch_others.size() << std::endl;
