...
void clustering_2trk_vtx2_ver3(std::multimap<int, stop_track>& tracks, int pl, std::ofstream& ofs);
...
int main(int argc, char** argv) {
	if (argc < 3) {
		...
		fprintf(stderr, "usage:prg in-mfile.all out-vtx.txt 1 --> vph&pid&stopflg&mcs&rng\n");
		exit(1);
	}
	...
	for (auto ev = set.begin(); ev != set.end(); ev++) {
		...
		if (mode == -1) {
			...
			clustering_2trk_vtx2_ver3(rid, rid.begin()->second.stoppl, ofs);
		}
		else {
	...
	for (auto itr0 = ret.begin(); itr0 != ret.end(); itr0++) {
		...
		for (auto itr1 = itr0->trk.begin(); itr1 != itr0->trk.end(); itr1++) {
			ofs << std::right << std::fixed
				<< std::setw(3) << std::setprecision(0) << itr1->second.chainid << " "
				<< std::setw(4) << std::setprecision(0) << itr1->second.pid << " "
				<< std::setw(2) << std::setprecision(0) << itr1->second.stop_flg << " "
				...
				<< std::setw(5) << std::setprecision(1) << itr1->second.mu_md << " "
				<< std::setw(5) << std::setprecision(1) << itr1->second.ip <<" "
