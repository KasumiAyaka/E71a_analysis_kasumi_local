...
void read_stop_txt(std::vector<Momentum_recon::Event_information>& momch, std::multimap<int, stop_track>& tracks) {
	...
	for (auto& ev : momch) {
		...
		for (auto& c : ev.chains) {
			...
			//if (int(c.particle_flg % 10000) == 13)continue;
...
void read_muon_mc_txt(std::vector<Momentum_recon::Event_information>& momch, std::multimap<int, stop_track>& tracks) {
	...
	for (auto& ev : momch) {
		...
		for (auto& c : ev.chains) {
			cnt++;
			auto trks = tracks.equal_range(ev.groupid);
			for (auto itr_trk = trks.first; itr_trk != trks.second; itr_trk++) {
				//if (int(itr_trk->second.pid%10000) != 13)continue;

				//std::cout << stop_tmp.groupid<<" "<< c.chainid << std::endl;
				if (c.base.size() == 1)continue;
				itr_trk->second.chainid = c.chainid;
				itr_trk->second.nseg = c.base.size();
				itr_trk->second.pl0 = c.base.begin()->pl;//dounstream
				itr_trk->second.pl1 = c.base.rbegin()->pl;//upstream
				itr_trk->second.npl = itr_trk->second.pl1 - itr_trk->second.pl0 + 1;
				itr_trk->second.pid = int(c.particle_flg % 10000);
				...
				itr_trk->second.pb = c.Get_proton_mcs_pb();

				if (itr_trk->second.pl1 <= itr_trk->second.stoppl) {//fwd
					//stop
					pos = itr_trk->second.pl1;
					itr_trk->second.rawid = c.base.rbegin()->rawid;
					itr_trk->second.ax = c.base.rbegin()->ax;
					itr_trk->second.ay = c.base.rbegin()->ay;
					itr_trk->second.x = c.base.rbegin()->x;
					itr_trk->second.y = c.base.rbegin()->y;
					itr_trk->second.z = c.base.rbegin()->z;
					itr_trk->second.vph = c.base.rbegin()->m[0].ph % 10000 + c.base.rbegin()->m[1].ph % 10000;
					itr_trk->second.vph2 = std::next(c.base.rbegin(), 1)->m[0].ph % 10000 + std::next(c.base.rbegin(), 1)->m[1].ph % 10000;
				...
				if (itr_trk->second.pl0 > itr_trk->second.stoppl) {//bwd
					//start
					itr_trk->second.rawid = c.base.begin()->rawid;
					itr_trk->second.ax = c.base.begin()->ax;
					itr_trk->second.ay = c.base.begin()->ay;
					itr_trk->second.x = c.base.begin()->x;
					itr_trk->second.y = c.base.begin()->y;
					itr_trk->second.z = c.base.begin()->z;
					itr_trk->second.vph = c.base.begin()->m[0].ph % 10000 + c.base.begin()->m[1].ph % 10000;
					itr_trk->second.vph2 = std::next(c.base.begin(), 1)->m[0].ph % 10000 + std::next(c.base.begin(), 1)->m[1].ph % 10000;
					if (c.base_pair.size() != 0) {
						auto itr = c.base_pair.begin();
						ax = itr->first.ax;
						ay = itr->first.ay;
						...
						itr_trk->second.dal = dal;
						itr_trk->second.dl = (dx * ay - dy * ax) / angle;
						itr_trk->second.dar = dar;
						itr_trk->second.dr = (dx * ax + dy * ay) / angle;
						itr_trk->second.d_pl = itr->second.pl - itr->first.pl;
					}
				}
