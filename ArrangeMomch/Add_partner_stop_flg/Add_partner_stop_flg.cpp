...
void add_stop_flg(std::vector<Momentum_recon::Event_information>& momch);
...
void add_stop_flg(std::vector<Momentum_recon::Event_information>& momch) {
	...
	for (auto& ev : momch) {
		...
		for (auto& c : ev.chains) {
			...
			if (c.base.begin()->pl > vpl) {
				// bwd
				if (c.base.rbegin()->pl > 130) {//penetrate
					c.stop_flg = 0;
				}
				if (c.base.rbegin()->x < 10000 || c.base.rbegin()->x  >240000) {//side out
					c.stop_flg = 0;
				}
				if (c.base.rbegin()->y < 10000 || c.base.rbegin()->y > 240000) {//side out
					c.stop_flg = 0;
				}
			}
			else {
				...
				if (c.base.begin()->x < 10000 || c.base.begin()->x > 240000) {//side out
					c.stop_flg = 0;
				}
				if (c.base.begin()->y < 10000 || c.base.begin()->y > 240000) {//side out
					c.stop_flg = 0;
				}
			}
		}
