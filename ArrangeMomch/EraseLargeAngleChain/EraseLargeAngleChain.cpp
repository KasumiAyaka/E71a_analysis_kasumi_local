...
void DivideMomch(std::vector<Momentum_recon::Event_information>& momch, std::string eraselog, double ang_thr);

int main(int argc, char** argv) {
	if (argc != 5) {
		fprintf(stderr, "usage:prg in.momch output.momch tan eraselog\n");
		exit(1);
	}
	std::string in_momch = argv[1];// input momch
	std::string out_momch = argv[2];// output momch
	double ang_thr = std::stod(argv[3]);//
	std::string out_txt = argv[4];// log of erased track
	...
	// erasechain
	DivideMomch(momch,out_txt, ang_thr);
	// write out
...
void DivideMomch(std::vector<Momentum_recon::Event_information>& momch, std::string eraselog, double ang_thr) {
	...
	ofs << "Angle thresold = " << ang_thr << std::endl;
	int count = 0;
	double tan,ax,ay;
	for (auto& ev : momch) {
		for (auto itr = ev.chains.begin(); itr != ev.chains.end();) {
			...
			if (fabs(ev.vertex_pl - itr->base.begin()->pl) > fabs(ev.vertex_pl - itr->base.rbegin()->pl)) {
			...
			if (tan > ang_thr) {
				...
				ofs << ev.groupid << " " << itr->chainid << std::endl;
				count++;
			}
