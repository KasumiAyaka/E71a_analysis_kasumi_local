// 2026/06/26
// kasumi
// Momentum_consistency_check
// compair ECC-mcs-momentum and BM-range-momentum
//途中
// 2026/06/26
...
void Conpare_eccmom_and_BMmom(std::vector<Momentum_recon::Event_information>&momch) {
	
	double mcs, rng;
	double mcserr[2],rngerr[2];
	for (auto& ev : momch) {
		...
		for (auto& c : ev.chains) {
			...
			mcs = c.ecc_mcs_mom[0];
			rng = c.bm_range_mom;
			mcserr[0] = c.ecc_mcs_mom_error[0][0];//-
			mcserr[1] = c.ecc_mcs_mom_error[0][1];//+
			rngerr[0] = c.bm_range_mom_error[0];//未実装
			rngerr[1] = c.bm_range_mom_error[1];//未実装
			if()
		}
	}
