...
void erase_momch(std::vector<Momentum_recon::Event_information>& momch0, std::vector<Momentum_recon::Event_information>& momch, int mat);

int main(int argc, char** argv) {
	if (argc !=4) {
		fprintf(stderr, "usage:prg in.momch output.momch PL\n");
		fprintf(stderr, "vpl > PL --> cut\n");
		exit(1);
	}
...
void erase_momch(std::vector<Momentum_recon::Event_information>& momch0, std::vector<Momentum_recon::Event_information>& momch, int mat) {
	...
	std::cout << "erase : vpl > " << mat << std::endl;

	int cnt = 0;
	for (auto& ev : momch0) {
		//std::cout << ev.groupid << " " << ev.vertex_pl << std::endl;
		;
		if (ev.vertex_pl > mat) {
			std::cout << ev.groupid << " " << ev.vertex_pl << std::endl;
			cnt++;
	...
	std::cout << "erased trk num = " << cnt << " / " << momch0.size() << std::endl;
}
